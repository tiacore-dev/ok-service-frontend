...
import {
  useAddProjectPlaceRelationsMutation,
  useDeleteProjectPlaceRelationMutation,
  useDeleteProjectPlaceRelationsMutation,
  useProjectPlaceRelationsQuery,
} from "../../queries/projectPlaceRelations";
...
export const ProjectPlaces = ({
  ...
  const places = React.useMemo(
    () =>
      (placesQuery.data ?? []).filter(
        (p) => p.object_id === objectId && !p.deleted,
      ),
    [placesQuery.data, objectId],
  );
  const relations = React.useMemo(
    () => (relationsQuery.data ?? []).filter((r) => r.project_id === projectId),
    [relationsQuery.data, projectId],
  );
  React.useEffect(() => {
    const ids = relations.map((r) => r.place_id);
    setSavedIds(ids);
    setDraftIds(ids);
  }, [relations]);
  const saving =
  ...
  const save = async () => {
    try {
      ...
      notification?.success({
        message: "Успешно",
        description: "Места спецификации обновлены",
      ...
      notification?.error({
        message: "Ошибка",
        description:
          error instanceof Error ? error.message : "Не удалось обновить места",
        placement: "bottomRight",
        duration: 2,
      });
    }
  ...
  if (loading)
    return (
      <section className="project__places-section">
        <Spin />
      </section>
    );
  if (placesQuery.isError || relationsQuery.isError)
    return (
      <section className="project__places-section">
        <Alert
          type="error"
        ...
        <div className="places-list">
          {relations.map((r) => (
            <div
              ...
              key={r.project_place_relation_id}
            >
              ...
              {canEdit && (
                <ActionDialog
                  ...
                  modalText={
                    <p>Вы уверены, что хотите удалить место из спецификации?</p>
                  }
                  onConfirm={() => remove(r.project_place_relation_id)}
                />
              )}
            </div>
          ))}
