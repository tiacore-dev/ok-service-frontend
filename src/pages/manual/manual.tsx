    ...
    <div className="manual">
      ...
      <div className="manual__content">
        <Typography>
          ...
          <Title id="status-logic" level={3}>
            Логика статусов
          </Title>

          <Title level={4}>Общая структура разделов</Title>
          <Paragraph>
            Разделы вложены друг в друга по принципу «матрёшки»:{" "}
            <Text strong>Объекты → Спецификация → Приёмка работ</Text>.
          </Paragraph>
          <ul>
            <li>В объект входит спецификация.</li>
            <li>В спецификацию добавляются работы в спецификации.</li>
            <li>В спецификацию входит приёмка работ.</li>
          </ul>
          <Paragraph>
            Каждый раздел имеет собственные статусы. Статус внешнего
            (родительского) раздела влияет на доступность действий и статусов
            внутреннего (дочернего) раздела.
          </Paragraph>
          ...
          <Paragraph>
            <Text strong>Спецификация:</Text>
          </Paragraph>
          <ul>
            <li>На согласовании</li>
            <li>В работе</li>
            <li>Работы выполнены</li>
            <li>Закрыто</li>
          </ul>
          <Paragraph>
            <Text strong>Работа в спецификации:</Text> согласовано / не
            согласовано.
          </Paragraph>
          <Paragraph>
            <Text strong>Приёмка работ:</Text>
          </Paragraph>
          <ul>
            <li>Предъявлено</li>
            <li>Выявлены нарушения</li>
            <li>Принято на объекте</li>
            <li>Документы подписаны</li>
          </ul>

          <Title level={4}>Логика работы по шагам</Title>
          <Paragraph>
            <Text strong>Шаг 1. Создание объекта.</Text> Новый объект получает
            статус «В ожидании».
          </Paragraph>
          <Paragraph>Можно:</Paragraph>
          <ul>
            <li>
              создать спецификацию — она автоматически получит статус «На
              согласовании»;
            </li>
            <li>
              отмечать работы в спецификации флагами «Согласовано» / «Не
              согласовано».
            </li>
          </ul>
          <Paragraph>Нельзя:</Paragraph>
          <ul>
            <li>создавать приёмку работ, пока объект находится в ожидании.</li>
          </ul>

          <Paragraph>
            <Text strong>Шаг 2. Объект переведён в статус «Действующий».</Text>{" "}
            Теперь доступны изменения статусов спецификации. Статусы меняются по
            порядку: на один шаг вперёд или назад.
          </Paragraph>
          <Paragraph>
            <Text strong>
              На согласовании → В работе → Работы выполнены → Закрыто
            </Text>
          </Paragraph>
          <Paragraph>
            Работы в спецификации могут быть согласованы частично или
            добавляться новые, пока спецификация не получит статус «Закрыто».
          </Paragraph>

          <Paragraph>
            <Text strong>Шаг 3. Создание приёмки работ.</Text>
          </Paragraph>
          <ul>
            <li>
              Если приёмки работ не создавались, спецификацию можно закрыть в
              любой момент.
            </li>
            <li>
              Если приёмки работ созданы, спецификацию можно перевести в статус
              «Закрыто» только после того, как все приёмки получат статус
              «Документы подписаны».
            </li>
          </ul>
          <Paragraph>
            Статусы приёмки меняются по одному шагу вперёд или назад:
          </Paragraph>
          <Paragraph>
            <Text strong>
              Предъявлено → Выявлены нарушения → Принято на объекте → Документы
              подписаны
            </Text>
          </Paragraph>

          <Paragraph>
            <Text strong>Шаг 4. Закрытие статусов.</Text> Финальные статусы
            редактирует только администратор:
          </Paragraph>
          <ul>
            <li>спецификация после статуса «Закрыто»;</li>
            <li>объект в статусе «Завершённый»;</li>
            <li>приёмка работ в статусе «Документы подписаны».</li>
          </ul>
          <Paragraph>Когда установлен финальный статус:</Paragraph>
          <ul>
            <li>
              ❌ Нельзя редактировать и добавлять записи в спецификацию,
              приёмки, места, материалы и другие связанные данные. Чтобы что-то
              дополнить, нужно вернуть спецификацию на шаг назад.
            </li>
            <li>⚠️ Вернуть из закрытого статуса может только администратор.</li>
            <li>
              ✅ Можно просматривать записи и материалы, а также скачивать и
              просматривать вложения.
            </li>
          </ul>

          <Title level={4}>Памятка: главное</Title>
          <ol>
            <li>Объект «В ожидании» — приёмки создавать нельзя.</li>
            <li>
              Объект «Действующий» — можно менять статусы спецификации по шагам.
            </li>
            <li>
              Спецификация закрывается либо сразу, либо после подписания
              документов по приёмке.
            </li>
            <li>
              Финальные статусы «Закрыто», «Завершённый» и «Документы подписаны»
              может вернуть только администратор.
            </li>
          </ol>
